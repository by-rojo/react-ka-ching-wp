...
if (! class_exists('contentstudio')) {
    ...
    class ContentStudio
    {
        protected $api_url = 'http://api.contentstudio.io/integrations/';
        ...
        public function add_menu()
        {
           if(current_user_can('editor') || current_user_can('administrator')){            
                add_menu_page('ContentStudio Publisher', 'ContentStudio', 'edit_posts', 'contentstudio_settings', [
                    $this,
                    'connection_page',
                ], 'https://storage.googleapis.com/lumotive-web-storage/favicon-16x16.png', '50.505');
        ...
        /**
         *
         * Get Image name based on the url
         *
         * @param $url string The url of the image
         * @return mixed  The name of the image
         */

        public function cstu_get_image_name($url)
        {
            $url_arr = explode('/', $url);
            $ct = count($url_arr);
            $name = $url_arr[$ct - 1];
        ...
        /**
         * Download all the images from gallery into uploads folder
         *
         * @param array $post_id The post id of the post which contains image url to be downloaded
         */

        public function cstu_download_images($post)
        {
            try {
                $images = $this->cstu_find_all_images_urls($post->post_content);
                foreach ($images as $image) {
                    $image_name = $this->cstu_get_image_name($image["url"]);
        ...
        public function cstu_verfiy_wp_user()
        {
            if (isset($_REQUEST['cstu_verfiy_wp_user']) && $_REQUEST['cstu_verfiy_wp_user']) {
                try {
                    if (isset($_REQUEST['username'], $_REQUEST['password']) && $_REQUEST['username'] && $_REQUEST['password']) {
                        $user = get_user_by('login', $_REQUEST['username']);
                        if ($user && $user->ID != 0) {
                            if (wp_check_password($_REQUEST['password'], $user->data->user_pass, $user->ID)) {
            ...
            try {
                if (isset($_REQUEST['cstu_set_token']) && isset($_REQUEST['token'])) {
                    ...
                    /*}
                        ...
                        // TODO: altenrative approach later on...
                ...
                echo json_encode(['status' => false, 'message' => self::UNKNOWN_ERROR_MESSAGE,
                    'line'=>$e->getLine(), 'error_message' =>  $e->getMessage()]);
            }
        ...
        {
            if (isset($_REQUEST['cstu_get_metadata']) && ($_REQUEST['cstu_get_metadata'])) {
            ...
            if (isset($_REQUEST) && isset($_REQUEST['cstu_create_new_post']) && isset($_REQUEST['token'])) {
                ...
                if ($valid) {
                    ...
                    if (isset($_REQUEST['post'])) {
                        $post_title = wp_strip_all_tags($_REQUEST['post']['post_title']);
                    ...
                    $tags = [];
                    if(isset($_REQUEST['post']['tags'])) $tags = $_REQUEST['post']['tags'];

                    // insert the post

                    $post_author = sanitize_text_field($_REQUEST['post']['post_author']);
                    ...
                    // download the image to the user server.
                    if ($get_post) $this->cstu_download_images($get_post);
                    ...
                    $this->uploadFeatureImages($post_response,$_REQUEST['post']['featured_image'],$post,$_REQUEST['post']['post_title']);
...
//            echo json_encode(['status' => false, 'message' => "Token is invalid."]);
//            die();
        }
        ...
        {
            if (isset($_REQUEST) && isset($_REQUEST['cstu_update_post']) && isset($_REQUEST['token'])) {
                ...
                if ($valid) {
                    ...
                    if (! $post or $post == 0) {
                        ...
                        global $wpdb;
                        $wpdb->update($wpdb->posts, ['post_title' => wp_strip_all_tags((string) $_REQUEST['post']['post_title'])], ['ID' => $post]);
                        // slug scenario
                    }
        ...
        public function cstu_generate_image($image_url, $post_id, $post_title)
        {
            ...
            try {
                ...
                $modified_filename = sanitize_title($post_title);

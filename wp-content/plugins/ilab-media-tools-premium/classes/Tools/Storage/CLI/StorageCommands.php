...
class StorageCommands extends Command {
	...
	public function migrate($args, $assoc_args) {
		...
		if ($media_cloud_licensing->is__premium_only()) {
			...
			if (isset($assoc_args['path-handling']) && in_array($assoc_args['path-handling'], ['replace', 'prepend'])) {
			...
			if (isset($assoc_args['verify']) ) {
				unset($assoc_args['verify']);
				$assoc_args['verify-migration'] = true;
			}
	...
	public function import($args, $assoc_args) {
		...
		if ($media_cloud_licensing->is__premium_only()) {
			...
			$pathMode = 'preserve';
			if (isset($assoc_args['preserve-paths']) && in_array($assoc_args['preserve-paths'], ['replace', 'prepend'])) {
				$pathMode = $assoc_args['preserve-paths'];
				if (empty(StorageToolSettings::prefixFormat())) {
					Command::Error("You have specified a path mode that requires a custom prefix, but you have not set one in Cloud Storage settings.");
					exit(1);
				}
			...
			$options = [
				'import-path' => empty($assoc_args['import-path']) ? null : $assoc_args['import-path'],
				'preserve-paths' => $pathMode,
				'import-only' => !empty($assoc_args['import-only']),
				'skip-thumbnails' => (!isset($assoc_args['skip-thumbnails']) || ($assoc_args['skip-thumbnails'] !== 'false'))
			];

			$task = new ImportFromStorageTask();
	...
	public function regenerate($args, $assoc_args) {
		...
		if ($media_cloud_licensing->is__premium_only()) {
			...
			$task = new RegenerateThumbnailTask();
			$this->runTask($task, $options);

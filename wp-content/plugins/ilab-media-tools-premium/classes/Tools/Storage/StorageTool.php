...
class StorageTool extends Tool {
	...
	public function __construct($toolName, $toolInfo, $toolManager) {
	    ...
	    if (!empty($toolInfo['storageDrivers'])) {
	        foreach($toolInfo['storageDrivers'] as $key => $data) {
		        ...
		        StorageToolSettings::registerDriver($key, $data['name'], $data['class'], $config, arrayPath($data, 'help', null));
            }
        }
		...
		if($this->enabled()) {
			if (is_admin()) {
				...
				if ((StorageToolSettings::driver() === 'do') && (strpos($this->client()->bucket(), '.') === false) && (!empty($this->client()->settings()->endPointPathStyle))) {
					...
					NoticeManager::instance()->displayAdminNotice('info', "You currently have the option <strong>Path Style Endpoint</strong> enabled.  You do not need to have this setting enabled with DigitalOcean unless your bucket name contains a period.  It is enabled by default, but you can turn it off in <a href='$settingsUrl'>Cloud Storage Settings</a>.", true, 'mcloud-do-path-endpoint-info', 365);
                }
			}
			...
			if (media_cloud_licensing()->is_plan__premium_only('basic')) {
				TaskManager::registerTask(MigrateTask::class);
				TaskManager::registerTask(UpdateURLsTask::class);
				TaskManager::registerTask(RegenerateThumbnailTask::class);
			}
			...
			if (empty($builtInImageOptimizer)) {
				foreach($this->toolInfo['compatibleImageOptimizers'] as $key => $plugin) {
					if (is_plugin_active($plugin)) {
						...
						} else if (($key == 'smush') || ($key == 'smush_pro')) {
			...
			if (version_compare($wp_version, '5.3', '>=')) {
				add_filter('wp_generate_attachment_metadata', [$this, 'handleGenerateAttachmentMetadata'], 1000, 3);
	            add_filter('wp_update_attachment_metadata', [$this, 'handleUpdateAttachmentMetadataFor53'], 1000, 2);
            } else {
			...
			if (ToolsManager::instance()->toolEnabled('debugging')) {
			    add_filter('load_image_to_edit_filesystempath', function($filepath, $attachment_id, $size) {
			        Logger::info("load_image_to_edit_filesystempath $filepath $attachment_id", [], __METHOD__, __LINE__);
			        return $filepath;
                }, 1, 3);


				add_filter('load_image_to_edit_attachmenturl', function($filepath, $attachment_id, $size) {
					Logger::info("load_image_to_edit_attachmenturl $filepath $attachment_id", [], __METHOD__, __LINE__);
					return $filepath;
				}, 1, 3);


				add_filter('load_image_to_edit_path', function($filepath, $attachment_id, $size) {
					Logger::info("load_image_to_edit_path $filepath $attachment_id", [], __METHOD__, __LINE__);
					return $filepath;
				}, 1, 3);
			...
			$this->hookupUI();
		} else if (!empty($this->client) && $this->client->settingsError()) {
	...
	public function handleGenerateAttachmentMetadata($meta, $id, $mode) {
    	Logger::info("Generate attachment metadata for $id for mode $mode", [], __METHOD__, __LINE__);
    ...
    public function handleUpdateAttachmentMetadataFor53($data, $id) {
        ...
        Logger::info("Update attachment metadata $id", [], __METHOD__, __LINE__);

	    // Uploads are handled by a different method for non-images
	    if (!wp_attachment_is_image($id)) {
	        $mimeType = get_post_mime_type($id);
	        if (!isset($data['sizes'])) {
		        Logger::info("Attachment $id - $mimeType is not an image", [], __METHOD__, __LINE__);
		        return $data;
            } else {
	            Logger::info("Attachment $id - $mimeType has sizes.", [], __METHOD__, __LINE__);
            }
	    }

	    // This means sizes haven't been generated yet.
	    if (!in_array($id, $this->generated) && empty($data['sizes'])) {
		    Logger::info("Attachment is missing sizes $id", [], __METHOD__, __LINE__);
		    return $data;
	    }
        ...
        } else if (empty($this->processingOptimized) && !in_array($id, $this->processed) && (arrayPath($_REQUEST, 'action') != 'image-editor')) {
		    Logger::info("Attachment hasn't been processed yet.", [], __METHOD__, __LINE__);
		    $this->processed[] = $id;
		    return $data;
	    }

	    Logger::info("Processing attachment metadata.", [], __METHOD__, __LINE__);
	 ...
	 * @return array
	 */
	public function uploadOriginalImage($data, $id, $preserveFilePaths = 'replace', $existingPrefix = null) {
		...
		if($this->client && $this->client->enabled()) {
            Logger::info("\tProcessing main file {$newData['file']}", [], __METHOD__, __LINE__);
	 ...
	 * @return array
	 */
    public function updateAttachmentMetadata($data, $id, $preserveFilePaths = 'replace', $skipThumbnails = false, $existingPrefix = null) {
        if($this->skipUpdate) {
            return $data;
        }
	    ...
	    Logger::info("Metadata:".json_encode($originalData, JSON_PRETTY_PRINT), [], __METHOD__, __LINE__);
        if(!file_exists($sourceFile)) {
            Logger::error("Missing $sourceFile", [], __METHOD__, __LINE__);
            return $originalData;
        }

        if (is_dir($sourceFile)) {
	        Logger::error("$sourceFile is directory.  Skipping.", [], __METHOD__, __LINE__);
	        return $originalData;
        }

        $sourceFileSize = filesize($sourceFile);
        if (filesize($sourceFile) < 512) {
	        Logger::error("File too small $sourceFile => $sourceFileSize bytes", [], __METHOD__, __LINE__);
	        return $originalData;
        } else {
	        Logger::info("File size $sourceFile => $sourceFileSize bytes", [], __METHOD__, __LINE__);
        }
	    ...
	    if ($media_cloud_licensing->is_plan__premium_only('basic')) {
		    if(empty(apply_filters('media-cloud/storage/ignore-optimizers', false, $id)) && ToolsManager::instance()->toolEnabled('optimizer')) {
			    OptimizerTool::instance()->startProcessing($id);
		    }
	    }
        ...
        if($this->client && $this->client->enabled()) {
            ...
            if($ignoreExistingS3 || !isset($data['s3'])) {
                Logger::info("\tProcessing main file {$data['file']}", [], __METHOD__, __LINE__);

                $doUpload = apply_filters('media-cloud/storage/upload-master', true);
                if (empty($forcedIgnoreExistingS3) || !isset($data['s3'])) {
                    $data = $this->processFile($upload_path, $data['file'], $data, $id, $preserveFilePaths, $doUpload, null, $existingPrefix, 'full');
                }
                ...
                if ($skipThumbnails && isset($data['sizes'])) {
                    ...
                    foreach($data['sizes'] as $key => $size) {
                        ...
                        if (($file == $data['file']) || ($sizedFileName == $data['file'])) {
                            ...
                            Logger::info("\tProcessing thumbnail {$size['file']}", [], __METHOD__, __LINE__);
                            $sizeData = $this->processFile($upload_path, $file, $size, $id, $preserveFilePaths, empty($forcedIgnoreExistingS3), null, $existingPrefix, $key);
	    ...
	    if ($media_cloud_licensing->is_plan__premium_only('basic')) {
		    if(empty(apply_filters('media-cloud/storage/ignore-optimizers', false, $id)) && ToolsManager::instance()->toolEnabled('optimizer')) {
			    $data = OptimizerTool::instance()->endProcessing($id, $data);
		    }
	    }
			...
			if(!isset($data['file'])) {
				...
				foreach($formats as $format) {
					if (in_array($format, $deletedFiles)) {
						Logger::info("File '$format' has already been deleted.", [], __METHOD__, __LINE__);
						continue;
					}
				...
				if(isset($data['sizes'])) {
					...
					foreach($data['sizes'] as $key => $size) {
                        ...
                        if (in_array($file, $deletedFiles)) {
					        Logger::info("File '$file' has already been deleted.", [], __METHOD__, __LINE__);
					        continue;
                        }
    ...
    private function extractPDFPageSize($upload_path, $file) {
	    ...
	    Logger::startTiming("PDF Loading $file with File Reader.", [], __METHOD__, __LINE__);
	    ...
	    fclose($pdfStream);
	    Logger::endTiming("PDF Loading $file with File Reader.", [], __METHOD__, __LINE__);

	    if (!$didParse) {
		    if (\WP_Image_Editor_Imagick::test()) {
			    Logger::startTiming("PDF Loading $file with Image Editor.", [], __METHOD__, __LINE__);
			    $imageEditor = wp_get_image_editor($upload_path.'/'.$file);
			    $sizes = $imageEditor->get_size();
			    $pdfWidth = $sizes['width'];
			    $pdfHeight = $sizes['height'];
			    Logger::endTiming("PDF Loading $file with Image Editor.", [], __METHOD__, __LINE__);
...
//	    }
	    ...
	    Logger::info("PDF Loading $file found dimensions: {$pdfWidth}w x {$pdfHeight}h.", [], __METHOD__, __LINE__);
	    return [
            $pdfWidth,
            $pdfHeight
        ];
	...
	public function handleUpload($upload, $context = 'upload') {
		...
		if($this->client && $this->client->enabled()) {
			...
			Logger::info("Adding $file to uploadedDocs", [], __METHOD__, __LINE__);
	...
	public function getAttachedFile($file, $attachment_id) {
		...
		if(!file_exists($file) && $shouldOverride) {
			...
			if(!$new_url) {
				...
				if($meta) {
					$new_url = $this->getAttachmentURLFromMeta($meta);
				} else if(!$meta && StorageToolSettings::docCdn()) {
...
//            }
			...
			do_action('media-cloud/storage/uploaded-attachment', $post_id, $file, $this->uploadedDocs[$fileKey]);
		} else {
			Logger::info("addAttachment - Missing '$fileKey' key on uploadeDocs.", [], __METHOD__, __LINE__);
	...
	public function getAttachmentURLFromMeta($meta) {
	    ...
	    if ($doSign) {
            ...
            if ((StorageToolSettings::driver() === 's3') && !empty(StorageToolSettings::signedCDN())) {
                return $url;
            } else if (!empty($cdn) && empty($ignoreCDN)) {
            ...
            try {
                ...
                Logger::error("Error trying to generate url for {$meta['s3']['key']}.  Message:".$ex->getMessage(), [], __METHOD__, __LINE__);
                return null;
            }
        }
	}
	 ...
	 * @throws \Exception
	 */
	public function processFile($upload_path, $filename, $data, $id = null, $preserveFilePath = 'replace', $uploadFile = true, $forcedAcl = null, $existingPrefix = null, $currentSize = null) {
		if(!file_exists($upload_path.'/'.$filename)) {
		    if (!empty(apply_filters('media-cloud/storage/report-missing-files', true))) {
			    Logger::error("\tFile $filename is missing.", [], __METHOD__, __LINE__);
            }
			return $data;
		}
        ...
        } else  if ($preserveFilePath == 'preserve') {
		...
		Logger::info("($preserveFilePath) Prefix => $prefix ", [], __METHOD__, __LINE__);
		...
		try {
            ...
            } else if (empty($this->settings->overwriteExisting)) {
			    if ($fileExists) {
		    ...
		    $privacy = apply_filters('media-cloud/storage/override-privacy', $privacy, $prefix.$bucketFilename, trailingslashit($upload_path).$filename, $data);
		    ...
		    if ($uploadFile || empty($url)) {
			    ...
			    if ($media_cloud_licensing->is_plan__premium_only('basic')) {
				    if(empty(apply_filters('media-cloud/storage/ignore-optimizers', false, $id)) && ToolsManager::instance()->toolEnabled('optimizer')) {
					    ...
					    $optResult = $optimizerTool->performOptimization($this->client, $id, $upload_path, $filename, $prefix, $bucketFilename, $privacy, $providerId, $currentSize);
					    if($optResult['status'] === OptimizerConsts::DEFERRED) {
						    return $data;
					    }

					    if($optResult['status'] >= OptimizerConsts::SUCCESS) {
						    $skipUpload = ($optResult['status'] === OptimizerConsts::SUCCESS_CLOUD);
						    if($skipUpload) {
							    $didOptimize = true;
							    $url = $optResult['results']->optimizedUrl();
						    }
					    }
				    }
			    }
                ...
                if (!$skipUpload) {
                    if (is_dir($upload_path.'/'.$filename)) {
                        Logger::error("File is a directory: {$upload_path}/{$filename}", [], __METHOD__, __LINE__);
                        return $data;
                    }

	                Logger::info("\tUploading $filename to S3.", [], __METHOD__, __LINE__);
	                $url = $this->client->upload($prefix.$bucketFilename, $upload_path.'/'.$filename, $privacy, StorageToolSettings::cacheControl(), StorageToolSettings::expires());
	                Logger::info("\tFinished uploading $filename to S3.", [], __METHOD__, __LINE__);
                }
            } else {
			    Logger::info("\tSkipping upload of $filename to S3.  Already exists.", [], __METHOD__, __LINE__);
		    }

		    $webpKey = null;
			$additionalPaths = apply_filters('as3cf_attachment_file_paths', ['file' => $upload_path.'/'.$filename], $id, $data);
		...
		if (!empty($canDelete) && StorageToolSettings::deleteOnUpload()) {
			if(file_exists($upload_path.'/'.$filename)) {
				...
				Logger::info("StorageTool::processFile - Adding $fileToDelete to delete cache.", [], __METHOD__, __LINE__);
	...
	private function deleteFile($file) {
		try {
			if($this->client && $this->client->enabled()) {
				Logger::info("StorageTool::deleteFile - Delete $file from storage.", [], __METHOD__, __LINE__);
				$this->client->delete($file);
			}
		}
		catch(StorageException $ex) {
			Logger::error("StorageTool::deleteFile - Error deleting file '$file'.", ['exception' => $ex->getMessage(), 'Key' => $file], __METHOD__, __LINE__);
		}
	}
	//endregion
...
TEMPLATE;
                ...
                <script>
                    jQuery(document).ready(function () {
                        ...
                        if (attachTemplate) {
                            ...
                            txt = txt.slice(0, idx) + '<?php echo str_replace("\n", "", $appendedTemplate);?>' + txt.slice(idx);
    ...
    public function regenerateFile($postId, $existingSizes = false) {
	    ...
	    Logger::startTiming('Regenerating metadata ...', ['id' => $postId], __METHOD__, __LINE__);
	    ...
	    Logger::endTiming('Regenerating metadata ...', ['id' => $postId], __METHOD__, __LINE__);
...
//
		...
		$data = $this->updateAttachmentMetadata($data, $postId, $pathmode, $skipThumbnails, $existingPrefix);
		if (isset($data['original_image'])) {
            $s3Data = $this->uploadOriginalImage($data, $postId, $pathmode, $existingPrefix);
            if (!empty($s3Data)) {
                $data['original_image_s3'] = $s3Data;
            }
        }
	...
	public function uploadUrlForFile($filename) {
		...
		if($this->client && $this->client->enabled()) {
			try {
				return $this->client->uploadUrl($prefix.$bucketFilename, StorageToolSettings::privacy(), StorageToolSettings::cacheControl(), StorageToolSettings::expires());
			}
			catch(StorageException $ex) {
				Logger::error('Generate File Upload URL Error', ['exception' => $ex->getMessage()], __METHOD__, __LINE__);
			}
		}
     ...
     * @throws StorageException
	 */
	public function importImageAttachmentFromStorage($fileInfo, $thumbs = []) {
		if(!$this->client || !$this->client->enabled()) {
			return null;
		}
	 ...
	 * @throws StorageException
	 */
	public function importExistingAttachmentFromStorage($postId, $fileInfo, $thumbs = [], $scaled = null) {
		if(!$this->client || !$this->client->enabled()) {
			return null;
		}

		if ($fileInfo->mimeType() && (strpos($fileInfo->mimeType(), 'image/') === 0)) {
			return $this->importExistingImageAttachmentFromStorage($postId, $fileInfo, $thumbs, $scaled);
		}
	 ...
	 * @throws StorageException
	 */
	public function importExistingImageAttachmentFromStorage($postId, $fileInfo, $thumbs = [], $scaled = null) {
		if(!$this->client || !$this->client->enabled()) {
			return null;
		}
	 ...
	 * @throws StorageException
	 */
	public function importAttachmentFromStorage($fileInfo, $thumbs = [], $scaled = null) {
		if(!$this->client || !$this->client->enabled()) {
			return null;
		}
	    ...
	    if ($skipThumbnails) {
		    foreach($tempFileList as $file) {
			    if (strpos(strrev($file), '/') !== 0) {
				    ...
				    } else if (preg_match('/.*-scaled\.[aA-zZ]+/', $file, $scaledMatch)) {
...
Optimizer;
        ...
        NoticeManager::instance()->displayAdminNotice($type, $message, true, $warningId);
    }
	...
	private function doImportFile($key, $thumbs, $scaled = null, &$newPostId = null) {
		...
		Logger::info("DIRECT URL ".$desturl, [], __METHOD__, __LINE__);
		Logger::info("DIRECT BASE ".$dir['baseurl'], [], __METHOD__, __LINE__);
				...
				if(preg_match('/([0-9]+x[0-9]+)\.(?:.*)$/', $thumb, $matches)) {
					if (!isset($indexedThumbs[$matches[1]])) {
						...
						if (!file_exists($thumbFile)) {
							...
							$client = new Client();
							$response = $client->get($url, ['save_to' => $thumbFile]);
		...
		if (!empty($scaled) && ($scaled !== $key)) {
			...
			if (!file_exists($scaledFile)) {
				...
				$client = new Client();
				$response = $client->get($url, ['save_to' => $scaledFile]);
			...
			foreach($sizes as $sizeKey => $size) {
				...
				if($resized) {
					...
					if(isset($indexedThumbs[$sizeIndex])) {
						...
						$meta['sizes'][$sizeKey] = ['file' => basename($indexedThumbs[$sizeIndex]), 'width' => $resized[4], 'height' => $resized[5], 'mime-type' => 'image/jpeg', 's3' => $sizeS3Info];
					}
				}
			}
        }
	...
	public function handleRegenerateFile() {
		if (!is_admin()) {
			wp_send_json(['status' => 'error', 'message' => 'Invalid security credentials.'], 400);
		}

		if (!isset($_POST['post_id'])) {
			wp_send_json(['status' => 'error', 'message' => 'Missing post ID.'], 400);
		}

		$postId = intval($_POST['post_id']);
		if (!current_user_can('edit_post',$postId)) {
			wp_send_json(['status' => 'error', 'message' => 'User is attempting to edit a post that they do not have access to.'], 400);
		}
...
MIGRATED;
			NoticeManager::instance()->displayAdminNotice('info', $migrationMessage, true, 'mcloud-migrated-other-plugin', 'forever');
		}
	}
	 ...
	 * @throws StorageException
	 */
	private function getOffloadS3URL(string $provider, $region, $bucket, $key) {
	...
	private function importStatelessMetadata($post_id, $meta, $statelessData) {
		...
		if ($hasMeta) {
			...
			update_post_meta($post_id, 'ilab_s3_info', ['s3' => $s3Info]);
		}
	...
	private function importOffloadMetadata($post_id, $meta, $offloadS3Data) {
		...
		if (empty($provider) || empty($bucket) || empty($key) || !in_array($provider, ['s3', 'aws', 'do', 'gcp'])) {
			return null;
		}

		$providerMap = ['aws' => 's3', 's3' => 's3', 'do' => 'do', 'gcp' => 'google'];
		...
		$s3Info = [
			'url' => $this->getOffloadS3URL($provider, $region, $bucket, $key),
			'provider' => $provider,
			'bucket' => $bucket,
			'key' => $key,
			'privacy' => 'public-read',
		...
		if (isset($meta['sizes'])) {
			...
			foreach($meta['sizes'] as $size => $sizeData) {
				...
				$sizeS3Info = $s3Info;
				$sizeS3Info['url'] = $this->getOffloadS3URL($provider, $region, $bucket, $sizeKey);
				$sizeS3Info['key'] = $sizeKey;
				$sizeS3Info['mime-type'] = $sizeData['mime-type'];
				$sizeData['s3'] = $sizeS3Info;
	...
	public function migrateFromOtherAjax() {
		...
		wp_send_json(['redirect' => admin_url('admin.php?page=media-cloud-task-manager')]);
	}
	 ...
	 */
    public function verifyPost($postId, $includeLocal, $reporter, $infoCallback) {
	    $client = new Client();
		    ...
		    if (strpos($mimeType, 'image') === 0) {
			    ...
			    if (!empty($sizes)) {
				    ...
				    foreach($sizes as $sizeKey => $sizeInfo) {
				        ...
				        if ($includeLocal) {
                            $this->getLocalUrl(function() use($postId, $uploadDir, $sizeKey, &$sizesData) {
	                            $url = wp_get_attachment_image_url($postId, $sizeKey);
	                            if (empty($url)) {
		                            return;
	                            }

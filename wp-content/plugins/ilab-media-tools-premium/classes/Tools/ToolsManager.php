...
final class ToolsManager {
    ...
    public function __construct() {
	    ...
	    if (!extension_loaded('mbstring')) {
		    NoticeManager::instance()->displayAdminNotice('warning',"Media Cloud recommends that the <code>mbstring</code> PHP extension be installed and activated.", true,'mcloud-no-mbstring');
	    }
        ...
        add_action('admin_enqueue_scripts', function(){
            wp_enqueue_script('mcloud-admin-js', ILAB_PUB_JS_URL . '/mcloud-admin.js', ['jquery', 'wp-util', 'wp-blocks', 'wp-element'], MEDIA_CLOUD_VERSION, true);
            wp_enqueue_script('ilab-settings-js', ILAB_PUB_JS_URL . '/ilab-settings.js', ['jquery', 'wp-util'], MEDIA_CLOUD_VERSION, true );
            wp_enqueue_style('ilab-media-cloud-css', ILAB_PUB_CSS_URL . '/ilab-media-cloud.css', null, MEDIA_CLOUD_VERSION );
        });
	    ...
	    add_action("updated_option", function ($setting, $oldValue=null, $newValue=null) {
	    ...
	    if (!defined('PHP_MAJOR_VERSION') || (PHP_MAJOR_VERSION<7)) {
		    NoticeManager::instance()->displayAdminNotice('warning', 'Media Cloud will stop supporting PHP 5 in the next major release of Media Cloud.  You should contact your hosting provider about upgrading.', true, 'media-cloud-php-56-warning', 30);
	    }

	    if (defined('MCLOUD_IS_BETA') && !empty(constant('MCLOUD_IS_BETA'))) {
		    $message = View::render_view('beta.beta-notes', []);
		    NoticeManager::instance()->displayAdminNotice('info', $message, true, 'mcloud-beta-notice'.MEDIA_CLOUD_VERSION, 'forever');
	    }
    }
...
//        MigrationsManager::instance()->displayMigrationErrors();
        ...
        if (!is_multisite() && ($media_cloud_licensing->is_plan__premium_only('multisite_basic', true) || $media_cloud_licensing->is_plan__premium_only('multisite_pro', true) || $media_cloud_licensing->is_plan__premium_only('multisite_unlimited', true))) {
            add_action( 'admin_notices', function () {
                ...
                <div class="notice notice-error">
                    <p><?php _e( "This license is only valid for multisite WordPress sites.  Please <a href='mailto:support@mediacloud.press'>contact us</a> for assistance in selecting the correct license.", 'ilab-media-tools' ); ?></p>
                </div>
                <?php
            } );
	    } else {
		    ...
		    if ($media_cloud_licensing->is_plan__premium_only('pro')) {
			    ToolsManager::registerTool("media-upload", include ILAB_CONFIG_DIR.'/media-upload.config.php');
		    }

	        if ($media_cloud_licensing->is_plan__premium_only('pro')) {
		        ToolsManager::registerTool("video-encoding", include ILAB_CONFIG_DIR.'/video-encoding.pro.config.php');
		        ToolsManager::registerTool("video-player", include ILAB_CONFIG_DIR.'/video-player.pro.config.php');
		        ToolsManager::registerTool("assets", include ILAB_CONFIG_DIR . '/assets.config.php');
	        } else {
	        ...
	        if ($media_cloud_licensing->is_plan__premium_only('basic')) {
		        ToolsManager::registerTool("optimizer", include ILAB_CONFIG_DIR . '/optimizer.config.php');
	        }
		    ...
		    if ($media_cloud_licensing->is_plan__premium_only('basic')) {
			    ToolsManager::registerTool("image-sizes", include ILAB_CONFIG_DIR . '/image-sizes.config.php');
		    }

		    if ($media_cloud_licensing->is_plan__premium_only('pro')) {
			    ToolsManager::registerTool("browser", include ILAB_CONFIG_DIR . '/browser.config.php');
			    ToolsManager::registerTool("integrations", include ILAB_CONFIG_DIR.'/integrations.config.php');
		    }
	...
	public function addMenus($networkMode, $networkAdminMenu) {
		...
		if ($media_cloud_licensing->is_plan__premium_only('pro')) {
			$this->isNetworkModeAdmin = ($networkMode && $networkAdminMenu);
			$this->isLocal = (!$networkMode && !$networkAdminMenu);
		} else {
			$networkMode = false;
		...
		if ($this->isNetworkModeAdmin) {
			...
			$this->networkTool->registerMenu('media-cloud', true, true, 'media-cloud');
			$this->wizard->registerMenu('media-cloud', $networkMode, $networkAdminMenu, 'media-cloud');
		}

		if ($this->isLocal || $this->isNetworkModeAdmin) {
			add_submenu_page('media-cloud', 'Media Cloud Settings', 'Settings', 'manage_options', 'media-cloud-settings', [$this, 'renderSettings']);
		}
		...
		foreach($this->tools as $key => $tool)  {
			...
			if ($key != 'troubleshooting') {
				add_settings_field("mcloud-tool-enabled-$key", $tool->toolInfo['name'], null,'media-cloud','ilab-media-features', ['key'=>$key]);
			}
		}

		$this->tools['troubleshooting']->registerMenu('media-cloud', $networkMode, $networkAdminMenu, 'media-cloud');
		...
		if ($this->isLocal || $this->isNetworkModeAdmin) {
			...
			if (!empty($this->pinnedTools)) {
				foreach($this->pinnedTools as $pinnedTool => $value) {
					...
					add_submenu_page( 'media-cloud', $tool->toolInfo['name'].' Settings', $tool->toolInfo['name'], 'manage_options', $pinnedSlug, [$this,'renderSettings']);
				}
			}
			...
			foreach($this->tools as $toolId => $tool) {
				if ($tool->envEnabled() && $tool->hasSettings() && !in_array($toolId, $displayedSettings)) {
					...
					add_submenu_page( 'media-cloud', $tool->toolInfo['name'].' Settings', $tool->toolInfo['name'], 'manage_options', 'media-cloud-settings-'.$toolId, [$this,'renderSettings']);
				} else if ($toolId == 'integrations') {
					$displayedSettings[] = $toolId;
					add_submenu_page( 'media-cloud', $tool->toolInfo['name'].' Settings', $tool->toolInfo['name'], 'manage_options', 'media-cloud-settings-'.$toolId, [$this,'renderSettings']);
				}
			}
			...
			foreach($this->tools as $toolId => $tool) {
				if ($tool->hasSettings() && !in_array($toolId, $displayedSettings)) {
					...
					add_submenu_page( 'media-cloud', $tool->toolInfo['name'].' Settings', null, 'manage_options', 'media-cloud-settings-'.$toolId, [$this,'renderSettings']);
				}
			}
		...
		foreach($this->tools as $key => $tool)  {
            ...
            if ($key != 'troubleshooting') {
                if ((($key === 'tasks') && $showTasks) || ($key !== 'tasks')) {
                    $tool->registerMenu('media-cloud', $networkMode, $networkAdminMenu, 'media-cloud');
                }
            }
		...
		add_submenu_page('media-cloud', 'Documentation', 'Documentation', 'manage_options', 'https://docs.mediacloud.press/');
		if (media_cloud_licensing()->is_plan('pro')) {
			add_submenu_page('media-cloud', 'Submit Issue', 'Submit Issue', 'manage_options', 'https://support.mediacloud.press/');
		}
		...
		if ($this->isLocal || $this->isNetworkModeAdmin) {
			if (LicensingManager::CanTrack()) {
				add_submenu_page('media-cloud', 'Opt-In Settings', 'Opt-In Settings', 'manage_options','media-cloud-settings-opt-in', [$this,'renderSettings']);
			}
		}
    ...
    public function insertSeparator($title = '', $parent = 'media-cloud') {
        if (empty($title)) {
            add_submenu_page( $parent, '', '<span class="ilab-admin-separator-container"><span class="ilab-admin-separator"></span></span>', 'manage_options', '#');
        } else {
            $safe_title = sanitize_title($title);
            add_submenu_page( $parent, '', '<span class="ilab-admin-separator-container ilab-admin-separator-'.$safe_title.'"><span class="ilab-admin-separator-title">'.$title.'</span><span class="ilab-admin-separator"></span></span>', 'manage_options', '#');
        }
    }
	...
	public function updateNetworkOptions() {
		...
		$url = add_query_arg(['updated' => 'true'], ($_POST['_wp_http_referer']));
		wp_redirect($url);
		exit;
	}
    ...
    public function renderSettings() {
	    ...
	    if (!is_multisite() && ($media_cloud_licensing->is_plan__premium_only('multisite_basic', true) || $media_cloud_licensing->is_plan__premium_only('multisite_pro', true) || $media_cloud_licensing->is_plan__premium_only('multisite_unlimited', true))) {
		    echo View::render_view( 'base/wrong-license', ['title' => 'Media Cloud '.MEDIA_CLOUD_VERSION]);
	    } else {
		    global $wp_settings_sections, $wp_settings_fields;

		    if(!empty($_GET['tab']) && in_array($_GET['tab'], array_keys($this->tools))) {
			    $tab = $_GET['tab'];
		    } else if (strpos($_GET['page'], 'media-cloud-settings-') === 0) {
    ...
    private function handlePinTool() {
        if (empty($_POST['tool'])) {
            json_response(['status' => 'error', 'message' => 'Missing tool parameter.']);
        }
        ...
        json_response(['status' => ($pinned) ? 'pinned' : 'unpinned', 'link' => admin_url("admin.php?page=media-cloud-settings&tool=$tool")]);
    }

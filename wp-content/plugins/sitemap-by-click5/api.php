...
function click5_sitemap_generate_CreateXML($name, $items, $custom_items = null) {
  ...
  foreach($items as $page) {
    ...
    /*$postDate = date('Y-m-d', strtotime($page->post_date));
    if($page_mod<$postDate)
    {
...
function click5_sitemap_generate_video_list($name) {
  ...
  //$results = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}posts WHERE id = 658 and post_status='publish' and (post_type != 'attachment' and post_type != 'acf-field' and post_type != 'acf-field-group' and post_type != 'revision' 
  //and post_type != 'acf' and post_type!='nav_menu_item' and post_type != 'oembed_cache' and post_type !='wpcf7_contact_form' and post_type !='wphb_minify_group') order by id desc", OBJECT );

  //$results = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}posts WHERE id = 20 and post_status='publish' and (post_type != 'attachment' and post_type != 'acf-field' and post_type != 'acf-field-group' and post_type != 'revision' 
  //and post_type != 'acf' and post_type!='nav_menu_item' and post_type != 'oembed_cache' and post_type !='wpcf7_contact_form' and post_type !='wphb_minify_group') order by id desc", OBJECT );
  ...
  foreach($results as $p_item) 
  {
        ...
        $content_array = explode(' ', $p_item->post_content);
          ...
          while(true)
          {
              if($ind<$length)
              {
                ...
                $matches[0][$ind] = str_replace('[/embed]', '', $matches[0][$ind]);
                //$matches[0][$ind] = str_replace('?feature=oembed', '', $matches[0][$ind]);
                ...
                /*if(function_exists("str_contains"))
                {
                  if(str_contains($matches[0][$ind], "?list="))
                  {
                    $matches[0][$ind] = substr($matches[0][$ind], 0, strpos($matches[0][$ind], "?list="));
                  }
                  if(str_contains($matches[0][$ind], "feature=oembed"))
                  {
                    $matches[0][$ind] = substr($matches[0][$ind], 0, strpos($matches[0][$ind], "feature=oembed"));
                  }
                  if(str_contains($matches[0][$ind], "?start"))
                  {
                    $matches[0][$ind] = substr($matches[0][$ind], 0, strpos($matches[0][$ind], "?start"));
                  }
                }
                */
    ...
    {       
     ...
     //$acf_fields2 = get_page_template();
     //$part_acf = get_template_part($acf_fields2);
     //$acf_fields3 = get_page_template_slug($p_item);
     //$part_acf = get_template_part(ABSPATH.'\\'. $acf_fields3);

...
define('CLICK5_SITEMAP_VERSION', '1.0.40');
...
function click5_sitemap_set_global_var()
{
  ...
  if (function_exists('glob')){
    ...
    foreach($files_xml as $xml_file) {
      ...
      if (function_exists('str_contains'))
      {
        if(!str_contains($xml_data, 'sitemap-by-click5')) {
          $detected_files++;
        }
      }
      else
      {
        if(strpos($xml_data, 'sitemap-by-click5') !== false) {
          $detected_files++;
        }
      }
...
function click5_sitemap_forceDefaultSettings() {
  ...
  */
  ...
  //update_option('click5_sitemap_display_post', false);
  //update_option('click5_sitemap_display_page', false);
  //update_option('click5_sitemap_display_cat_tax', false);
  //update_option('click5_sitemap_display_tag_tax', false);
  //update_option('click5_sitemap_display_authors_tax', false);
                      ...
                      <?php 
                        ...
                        foreach($taxonomies_array as $taxonomies_item) {
                          if(boolval(esc_attr(get_option('click5_sitemap_display_' . $taxonomies_item)))) {
                            ?>
                              <option value="<?php echo esc_attr($taxonomies_item); ?>"><?php echo esc_attr(ucfirst($taxonomies_item)); ?></option>
                            <?php
                          }
                        }
      ...
      <div id="post-body-content">
        <div class="error"<?php  global $global_yoast_xml; if(!$global_yoast_xml) {echo "style='display:none'";} else { echo 'style="margin-top: 0px!important;"';} ?>>
          <p>
            ...
            <?php _e('We have detected an XML sitemap generated by different plugin. We recommend to deactivate the XML sitemap generated by the YOAST SEO plugin in order to prevent any conflicts or SEO errors on your website. These may be reported in Google Search Console and other tools.', 'sitemap-by-click5'); ?>
          </p>
        ...
        <div class="error" <?php  global $global_aio_seo_xml; if(!$global_aio_seo_xml) {echo "style='display:none'";} else { echo 'style="margin-top: 0px!important;"';} ?>>
          <p>
            <b><?php _e('Warning: ', 'sitemap-by-click5'); ?></b>
            <?php _e('We have detected an XML sitemap generated by different plugin. We recommend to deactivate the XML sitemap generated by the All In One SEO plugin in order to prevent any conflicts or SEO errors on your website. These may be reported in Google Search Console and other tools.', 'sitemap-by-click5'); ?>
          </p>
        ...
        <div class="error" <?php  global $global_file_xml; if(!$global_file_xml) {echo "style='display:none'";} else { echo 'style="margin-top: 0px!important;"';} ?>>
          <p>
            <b><?php _e('Warning: ', 'sitemap-by-click5'); ?></b>
            <?php _e('We have detected an XML sitemap generated by different plugin. We recommend to deactivate the other sitemap in order to prevent any conflicts or SEO errors on your website. These may be reported in Google Search Console and other tools.', 'sitemap-by-click5'); ?>
          </p>
        </div>
        <div class="postbox">

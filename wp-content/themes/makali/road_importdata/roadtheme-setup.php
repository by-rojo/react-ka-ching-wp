...
class RoadthemeSetup {
	...
	function installplugin_function ()  {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['plugin_slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( ' Activating.....', 'makali' ),
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['plugin_slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( ' Updating......', 'makali' ),
					'loading_url' => $loading_url
				);
				break;
			}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['plugin_slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( ' Installing ......','makali'),
					'loading_url' => $loading_url
				);
				break;
			}
		...
		if ( $json ) {
			...
			wp_send_json( array( 'done' => 1, 'message' => esc_html__( ' Success', 'makali' ),'loading_url' => $loading_url_success ) );
		}
	...
	function roadtheme_register_required_plugins() {
		...
		$plugins = array(
			...
			// Plugins from the WordPress Plugin Rmakalitory.
			array(
			...
			array(
				'name'      => esc_html__('Instagram Feed', 'makali'),
				'slug'      => 'instagram-feed',
				'required'  => true,
			),
			array(

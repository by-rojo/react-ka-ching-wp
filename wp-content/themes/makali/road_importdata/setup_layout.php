...
class RoadthemeLayout {
	...
	<form action="" method="post"><?php
				...
				if ( $count ) :
					?><h2><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed or updated', 'makali' ); ?></h2>
					<ul class="roadthemez-plugins"><?php
						...
						foreach ( $plugins['all'] as $slug => $plugin ) :
							?><li data-slug="<?php echo esc_attr( $slug ); ?>">
								...
								?></span><div class="<?php echo esc_attr($action_class);?>"></div><div class="spinner"></div>
							</li><?php
						endforeach;
					?></ul><?php
				else :
					?><p class="lead success"><strong><?php esc_html_e( ' All plugins has already been installed and up to date.', 'makali' ) ?></strong></p><?php
				endif;
				?>
				<p class="roadtheme_plugins">
					<?php 	if ( $count ) : ?><a href="#" class="button-primary button button-large button-next install" ><?php esc_html_e( 'Install plugins', 'makali' ); ?></a><?php endif; ?>
					<a href="#install_demo" class="button-primary button button-large button-next demo_data" ><?php esc_html_e( 'Install Demo Data', 'makali' ); ?></a>
				</p>
			</form>
							...
							<section class="content">
								...
								?>
									...
									<div class="content-inner">
										...
										?>
											<div class="r_item <?php echo esc_attr($demo_data) .' '.$is_active; ?>"  >
												<div class="item-inner">
													<div class="image">
														<img src="<?php echo THEME_URI; ?>/road_importdata/images/<?php echo esc_attr($val['image']);?>.png" class="pull-right1" />	
														<?php if(isset($home_page->ID)) { ?>
															<label><?php echo ('imported');?></label>
														<?php } ?> 
													</div>	
													<h3 class="name"><?php echo esc_attr($val['page_name']); ?></h3>
													<form action="" method="post" onsubmit="doSubmit(this)">  
														...
														<input type="hidden" value="<?php echo esc_attr($key); ?>" name="import_data" /> 
													</form>	 
												</div>	
											</div>				
										<?php } ?>
	...
	static function load_system_requiment() {
		...
		<ul class="system_requirement">
			<li>
				<span><b><?php esc_html_e('PHP Version: ', 'makali'); echo PHP_VERSION; ?></b></span>
				<span class="system_icon<?php echo esc_attr(self::check_field_status(PHP_VERSION, '5.6')); ?>"></span>		</br>
				<span><a href="https://wordpress.org/support/article/requirements/"><?php esc_html_e('Requiment ', 'makali'); ?></a></span>	
			</li>
			<li>
				<b><?php esc_html_e('Memory Limit: ', 'makali'); echo ini_get('memory_limit') ;  ?></b>
				<span><?php esc_html_e('Memory Limit requires at least 256M of memory ', 'makali');   ?></span>
			...
			<li>
				<b><?php esc_html_e('Max Execution Time: ', 'makali'); echo esc_attr($max_execution_time) ;  ?></b>
				<span><?php esc_html_e('Max Execution Time requires at least 300s of PHP execution time: ', 'makali'); ?></span>
				<span class="system_icon<?php echo esc_attr(self::check_field_status($max_execution_time, 300)); ?>"></span>				
			</li>
			<li>
				<b><?php esc_html_e('Max Upload File Size: ', 'makali'); echo esc_attr($file_size) ;  ?></b>
				<span><?php esc_html_e('Max Upload File Size requires at least 2M of total upload file size: ', 'makali'); ?></span>
				<span class="system_icon<?php echo esc_attr(self::check_field_status($file_size, 2*MB_IN_BYTES)); ?>"></span>				
			</li>
			<li>
				<b><?php esc_html_e('Server Max Post Size: ', 'makali'); echo esc_attr($post_max_size_limit) ;  ?></b>
				<span><?php esc_html_e('Server Max Post Size requires at least 8M of total file size: ', 'makali'); ?></span>
...
RoadthemeLayout::load_demo_data();
	...
	esc_html_e('You need fix all error in required Sytem to can continue setup ', 'makali');
}
?>
<script type="text/javascript">
